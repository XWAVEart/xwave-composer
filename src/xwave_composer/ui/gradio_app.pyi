...
class OutputJpegImage(gr.Image):
    """Like gr.Image, but the download button saves a high-quality JPEG."""

    def __init__(self, *args, **kwargs):
        kwargs.setdefault("format", "jpeg")
        super().__init__(*args, **kwargs)

    def postprocess(self, value: np.ndarray | Image.Image | str | Path | None) -> ImageData | None:
        if value is None:
            return None
        if self.watermark and self.watermark.watermark is not None:
            from gradio.image_utils import add_watermark

            value = add_watermark(value, self.watermark)

        if isinstance(value, (str, Path)):
            path = Path(value)
            return ImageData(path=str(path), orig_name=path.name)
        ...
        return ImageData(path=str(path), orig_name="xwave_output.jpg")
...
def render_layers_html(session: ComposerSession) -> str:
    ...
    for obj in reversed(session.doc.objects):  # top-most first
        ...
        label = html_lib.escape(text[:72])
        lid = html_lib.escape(obj.id)
        cards.append(
            f'<div class="xwave-card{sel}{empty}" data-layer-id="{lid}" draggable="true">'
...
def _inspector(session: ComposerSession) -> dict[str, Any]:
    ...
    if obj is None:
        return {"kind": "none", "prompt": "", "opacity": 1.0, "iso": "", "raw": None}
    return {
        "kind": "object",
...
def build_app(config: AppConfig | None = None) -> gr.Blocks:
    ...
    def mark_output_dirty() -> None:
        ...
        delay = float(config.get("sdxl_hyper", "live_debounce_s", default=0.35))
    ...
    def pack(run_out: bool = True, sync_out: bool = False) -> tuple:
        ...
        insp_prompt, insp_iso, insp_opacity, raw_view, prompt_view, llm_prompt_view"""
        work = _work(session)
        if sync_out:
            out = run_output_now()
        else:
    ...
    with gr.Blocks(title="xwave-composer", elem_classes=["xwave-app"]) as demo:
        ...
        with gr.Row(elem_classes=["xwave-row", "xwave-topbar"], equal_height=True):
            ...
            with gr.Column(scale=1, min_width=380, elem_classes=["xwave-col"]):
                with gr.Row(elem_classes=["xwave-bar-row", "xwave-knob-row"]):
                    ...
                    export_steps = gr.Slider(
                        0, 40,
                        value=int(config.get("export", "default_refine_steps", default=8)),
                        step=1,
                        label="Refine",
                        scale=2,
                        min_width=84,
                    ...
                    export_btn = gr.Button(
                        "Export 2×", variant="primary", size="sm", scale=0, min_width=88,
            ...
            with gr.Column(scale=3, min_width=420, elem_classes=["xwave-col", "xwave-props-col"]):
                with gr.Row(elem_classes=["xwave-props-grid"]):
                    ...
                    with gr.Column(scale=1, min_width=250, elem_classes=["xwave-panel"]):
                        ...
                        export_path = gr.Textbox(label="Export path", interactive=False, lines=1)
        ...
        def on_action(payload, den, steps, cfg_v, eta_v, llm, neg, oseed):
            ...
            if atype == "transform":
                ...
                mark_output_dirty()
                # Keep WORK canvas HTML stable mid-drag; timer refreshes OUTPUT.
                return tuple(gr.update() for _ in pack_out)
        ...
        def on_params(den, steps, cfg_v, eta_v, llm, neg, oseed):
            apply_settings(den, steps, cfg_v, eta_v, llm, neg, oseed)
        ...
        def on_export(steps):
            try:
                img, path = session.export_final(refine_steps=int(steps) if steps else None)
                return img, str(path), session.status
            except Exception as exc:  # noqa: BLE001
        ...
        for comp in (denoise, out_steps):
            comp.release(on_params, inputs=settings_in, outputs=[status], show_progress="hidden")
        for comp in (cfg, eta, out_seed):
            comp.input(on_params, inputs=settings_in, outputs=[status], show_progress="hidden")
        neg_prompt.blur(on_params, inputs=settings_in, outputs=[status], show_progress="hidden")
        ...
        export_btn.click(on_export, inputs=[export_steps], outputs=[export_image, export_path, status])
        ...
        vram_timer.tick(render_vram_html, outputs=[vram_html], show_progress="hidden")
